...
async def loop(evaluate, X0, archive0, params, hooks):
    ...
    hook_gp = itemgetter('gp')(hooks)
    hook_acqu = itemgetter('acqu')(hooks)
...
#         Y_archive1_opt = np.min(Y_archive1_mean, axis=0)
        ...
        for _xi in xi_list:
            def gp_evaluate(X, hook=None, _xi=_xi):
                return -expected_improvement(X, gp, Y_archive1_opt, _xi)
            problems.append(gp_evaluate)
    ...
    if hook_acqu:
        await hook_acqu(None, None, 0)
    
    X1, Y1 = await get_best_solutions(task)
    x = X1.flatten()[0]
    y = Y1.flatten()[0]
    
    if hook_acqu:
        await hook_acqu(x, y, 1)
